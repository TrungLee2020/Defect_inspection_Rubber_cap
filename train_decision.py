...
from models_final import SegmentNet, DecisionNet, weights_init_normal
from dataset import RubberDataset
import numpy as np
...
if opt.begin_epoch != 0:
    # Load Pretrain model
    decision_net.apply(Module.load_state_dict(torch.load("./saved_models/segment_net_%d.pth" % (opt.begin_epoch))))
else:
    # Init weights
    decision_net.apply(weights_init_normal)
# Load pretrain segment
segment_net.apply(Module.load_state_dict(torch.load("./saved_models/segment_net_%d.pth" % (opt.begin_epoch))))
...
trainOKloader = DataLoader(
    RubberDataset(dataSetRoot, transforms_=transforms_, transforms_mask=transforms_mask, subFold="Train_OK", isTrain=True),
...
trainNGloader = DataLoader(
    RubberDataset(dataSetRoot, transforms_=transforms_, transforms_mask=transforms_mask, subFold="Train_NG", isTrain=True),
    batch_size=opt.batch_size,
    shuffle=True,
    num_workers=opt.worker_num,
)
...
testloader = DataLoader(
    RubberDataset(dataSetRoot, transforms_, transforms_mask, subFold="Test", isTrain=False),
    batch_size=1,
    shuffle=False,
    num_workers=opt.worker_num,
)
...
for epoch in range(opt.begin_epoch, opt.end_epoch):
    ...
    lenNum = min(len(trainOKloader), len(trainNGloader))
    lenNum = 2 * (lenNum - 1)
    ...
    if opt.need_test and epoch % opt.test_interval == 0 and epoch >= opt.test_interval:
        for i, testBatch in enumerate(testloader):
            ...
            save_image(segTest.data, "%s/img_%d_%s.jpg"%(save_path_str, i, labelStr))
