...
class TensorImages:
    def __init__(self, imgs_input, imgs_pred, vmin, vmax, method, dtype="float64", filenames=None):
        ...
        # if grayscale, reducr dim to (samples x length x width)
...
def label_images(images_th):
    ...
    for i, image_th in enumerate(images_th):
        ...
        # append image
        image_labeled[i] = image_labeled

        # compute areas of anomalous regions n the current image
